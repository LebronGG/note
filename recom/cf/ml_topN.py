...
def load_file(file=10):

    filename_user_movie = './dataset/ml-{}k/ratings.dat'.format(file)
    user_movie = {}
    for line in open(filename_user_movie,'r',encoding='utf-8'):
...
def getRecommendations(user, user_movie, movies, userSim, N):
    ...
    # top-10 pred
    pred = sorted(pred.items(), key=itemgetter(1), reverse=True)[0:N]
    return pred

if __name__ == "__main__":

    user_movie, movies = load_file(100)

    userSim = calUserSim(user_movie)

    # Recommend
    pred = getRecommendations('182', user_movie, movies, userSim, 3)
