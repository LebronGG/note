...
@logger('读取文件')
def read_ml(relative_path, separator='::'):
    data = []
    with open(relative_path, 'r') as f:
        for line in f.readlines():
            values = line.strip().split(separator)
            user_id, item, price = int(values[0]), int(values[1]), float(values[3])
            data.append((user_id, item, price))
    return data
...
def neaten_id(data: List[tuple]) -> Tuple[List[Tuple[int, int, int]], int, int, dict, dict]:
    ...
    for user_id_old, item_id_old, count in data:
        ...
        if item_id_old not in item_id_old2new:
            ...
            n_item += 1
        new_data.append((user_id_old2new[user_id_old], item_id_old2new[item_id_old], count))
...
def pack_data(data,split_test_ratio=0.2,shuffle_before_split=True,split_ensure_positive=False,topk_sample_user=300):

    data = [(d[0], d[1], d[2]) for d in data]  # 变成隐反馈数据
