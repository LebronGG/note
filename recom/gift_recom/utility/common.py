...
from utility.evaluation import TopkData, topk_evaluate
...
def topk(topk_data: TopkData, score_fn: Callable[[Dict[str, List[int]]], List[float]], N=[10, 20]):
    precisions, recalls = topk_evaluate(topk_data, score_fn, N)
    for k, precision, recall in zip(N, precisions, recalls):
        f1 = 2. * precision * recall / (precision + recall) if precision + recall else 0
        print('[N=%d, precision=%.3f%%, recall=%.3f%%, f1=%.3f%%]' %
