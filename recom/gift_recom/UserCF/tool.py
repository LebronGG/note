...
def user_similarity(train_data: list, n_user: int, n_item: int) -> List[List[float]]:
    ...
    for user_id, item_id, count in train_data:
...
@logger('开始计算用户物品评分矩阵，', ('K',))
def user_item_score(train_data: list, n_user: int, n_item: int, W: List[List[float]], K) -> List[List[float]]:
    ...
    for user_id in range(n_user):
        ...
        for similar_user_id, similarity_factor in sorted(Wu.items(), key=lambda x: x[1], reverse=True)[:K]:
